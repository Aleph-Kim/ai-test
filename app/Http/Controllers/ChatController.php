...
class ChatController extends Controller
{
    ...
    // 모델이 답변 끝에 붙이는 꼬리 질문 블록의 시작 표시 (화면에는 버튼으로만 노출)
    private const FOLLOW_UP_MARKER = '[[꼬리질문]]';

    private const MAX_FOLLOW_UPS = 4;
    ...
    private const INSTRUCTIONS = <<<'TXT'
        ...
        - 질문이 넓어서 근속 기간, 퇴직 사유, 고용 형태 등 상황에 따라 답이 달라지면 모든 경우를 나열하지 않습니다.
          핵심 개요만 2~3문장으로 답한 뒤, 상황을 좁히는 꼬리 질문을 2~4개 제시합니다.
        - 꼬리 질문은 답변 맨 끝에 아래 형식으로만 씁니다. 직원이 그대로 눌러 보낼 문장이므로 직원 입장의 짧은 서술로 씁니다.
          [[꼬리질문]]
          - 근속 기간이 1년 미만이에요
          - 자발적으로 퇴사하는 경우예요
        - 질문이 이미 구체적이면 꼬리 질문을 쓰지 않습니다.
        TXT;
    ...
    public function messages(Request $request, Conversation $conversation): JsonResponse
    {
        ...
        return response()->json(
            $conversation->messages()->orderBy('id')->get(['role', 'content', 'citations', 'follow_ups', 'provider', 'model', 'elapsed_ms', 'created_at'])
        );
    }
    ...
    public function ask(Request $request, Conversation $conversation): StreamedResponse
    {
        ...
        return response()->eventStream(function () use ($conversation, $question, $history, $startedAt) {
            ...
            [$content, $followUps] = $this->splitFollowUps($answer);
    ...
    /**
     * 답변 본문과 끝에 붙은 꼬리 질문 목록 분리
     *
     * @return array{0: string, 1: list<string>}
     */
    private function splitFollowUps(string $answer): array
    {
        $position = mb_strpos($answer, self::FOLLOW_UP_MARKER);
        if ($position === false) {
            return [trim($answer), []];
        }

        $block = mb_substr($answer, $position + mb_strlen(self::FOLLOW_UP_MARKER));
        $followUps = collect(preg_split('/\R/u', $block))
        ...
        return [trim(mb_substr($answer, 0, $position)), $followUps];
    }
